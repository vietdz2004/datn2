...
exports.getAllOrders = async (req, res) => {
  try {
    const { page = 1, limit = 10, search = '', trangThaiDonHang = '' } = req.query;
    const offset = (page - 1) * limit;
    
    console.log('📋 Admin order query params:', { search, status: req.query.status, trangThaiDonHang, page, limit });
    ...
    if (trangThaiDonHang.trim()) {
      whereClause += ` AND o.trangThaiDonHang = ?`;
      params.unshift(trangThaiDonHang.trim());
    }
  ...
  try {
    ...
    const [result] = await sequelize.query(`
      INSERT INTO donhang (
        ...
        tenNguoiNhan, soDienThoai, diaChiGiaoHang, ghiChu, email, maDonHang
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, {
      replacements: [
        ...
        orderData.tenNguoiNhan, orderData.soDienThoai, orderData.diaChiGiaoHang,
